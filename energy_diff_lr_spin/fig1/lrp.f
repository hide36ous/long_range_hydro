c
      parameter(nsite=2**12,ntrans=10**4,nth=1,nmc=3,nti=10,
     & maxtime=400,nno=10**3)
...
c      
      do i=1,nsite
       ...
       do kk=1,nsite/2
        ...
        do lp=kk,nsite/2   
         acurr=-ah*aj*wgt(lp)/zr*
     &    (sy(ip(ik,lp),ia)*sz(ik,ia)-sy(ik,ia)*sz(ip(ik,lp),ia))/2.d0
...
c
      ajt=0.d0;ajtt=0.d0;
      do j=0,nsite/2;
         ajt=ajt+ajeje(j,0)/dble(nmc)
         ajtt=ajtt+ajejet(j,200)/dble(nmc);
      end do
      write(70,111) alpha,dble(nsite),ajt,ajtt;      
c
      do j=0,nsite/2
