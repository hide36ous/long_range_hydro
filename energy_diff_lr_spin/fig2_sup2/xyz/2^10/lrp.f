c
      parameter(nsite=2**10,mc=50,nper=10**3,ntrans=10**3,nno=10**3)
...
c         
      do i=1,nsite
       ...
       do kk=1,nsite/2
        ...
        do lp=kk,nsite/2   
c         acurr=-ah*(2.d0*aj)/2.d0*wgt(lp)/zr*
c     &    (sy(ip(ik,lp),ia)*sz(ik,ia)-sy(ik,ia)*sz(ip(ik,lp),ia))
c         aje(i)=aje(i)+acurr
         do imm=1,nsite
     ...
     &     -ajz*ajx/4.d0*
     &     (wgtd(ik,imm)/zr*wgtd(ip(ik,lp),imm)/zr*sy(imm,ia)*
     &      (sz(ik,ia)*sx(ip(ik,lp),ia)-sz(ip(ik,lp),ia)*sx(ik,ia))
     &     -wgt(lp)/zr*wgtd(imm,ik)/zr*sy(ik,ia)*
     &      (sz(ip(ik,lp),ia)*sx(imm,ia)-sz(imm,ia)*sx(ip(ik,lp),ia))
     &     +wgtd(imm,ip(ik,lp))/zr*wgt(lp)/zr*sy(ip(ik,lp),ia)*
     &      (sz(imm,ia)*sx(ik,ia)-sz(ik,ia)*sx(imm,ia)) )       
     &     -ajx*ajy/4.d0*
     &     (wgtd(ik,imm)/zr*wgtd(ip(ik,lp),imm)/zr*sz(imm,ia)*
     &      (sx(ik,ia)*sy(ip(ik,lp),ia)-sx(ip(ik,lp),ia)*sy(ik,ia))
     &     -wgt(lp)/zr*wgtd(imm,ik)/zr*sz(ik,ia)*
     &      (sx(ip(ik,lp),ia)*sy(imm,ia)-sx(imm,ia)*sy(ip(ik,lp),ia))
     &     +wgtd(imm,ip(ik,lp))/zr*wgt(lp)/zr*sz(ip(ik,lp),ia)*
