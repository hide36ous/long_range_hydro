c
      parameter(nsite=2**9,ntrans=10**4,mmax=7,nti=mmax*8*10**3,
     & nth=10**1,nmc=700,nno=10**3)
...
c
       al=dsqrt(ahy_eff**2+ahz_eff**2+ah**2)
       alxy=dsqrt(ahy_eff**2+ah**2)
       asint=alxy/al; acost=ahz_eff/al
       acosp=ah/alxy; asinp=ahy_eff/alxy
c       
...
c p1
      call symp(nsite,sx,sy,sz,im,ip,wgt,ap1*at,alpha,zr,ajy,ajz,ia)
c p2
      call symp(nsite,sx,sy,sz,im,ip,wgt,ap1*at,alpha,zr,ajy,ajz,ia)
c p3      
      call symp(nsite,sx,sy,sz,im,ip,wgt,ap3*at,alpha,zr,ajy,ajz,ia)
c p4      
      call symp(nsite,sx,sy,sz,im,ip,wgt,ap1*at,alpha,zr,ajy,ajz,ia)
c p5      
      call symp(nsite,sx,sy,sz,im,ip,wgt,ap1*at,alpha,zr,ajy,ajz,ia)
...
c
************************************************************************
      subroutine symp(nsite,sx,sy,sz,im,ip,wgt,at,alpha,zr,ajy,ajz,ia)
      implicit real*8 (a-b,e-h,o-z)
      implicit complex*16 (c-d)
...
c
c      do i=1,nsite
c         sy1(i)=dcos(thex)*sy(i,ia)-dsin(thex)*sz(i,ia)
c         sz1(i)=dsin(thex)*sy(i,ia)+dcos(thex)*sz(i,ia)
c      end do
c
c     theta_zz(t)
c      
      do i=1,nsite
         thez(i)=0.d0   
         do lr=1,nsite/2
            thezii=wgt(lr)/zr*ajz*(sz(ip(i,lr),ia)+sz(im(i,lr),ia))*at/2
            thez(i)=thez(i)+thezii
         end do         
      end do
c
      do i=1,nsite
         sx1(i)=dcos(thez(i))*sx(i,ia)-dsin(thez(i))*sy(i,ia)
         sy1(i)=dsin(thez(i))*sx(i,ia)+dcos(thez(i))*sy(i,ia)
      end do
c
c     theta_yy(t+dt)
c
      do i=1,nsite
         they(i)=0.d0   
         do lr=1,nsite/2
            theyii=wgt(lr)/zr*ajy*(sy1(ip(i,lr))+sy1(im(i,lr)))*at
            they(i)=they(i)+theyii
         end do         
      end do
c
      do i=1,nsite
         sx2(i)=dcos(they(i))*sx1(i)+dsin(they(i))*sz(i,ia)
         sz(i,ia)=-dsin(they(i))*sx1(i)+dcos(they(i))*sz(i,ia)
      end do
c
c     theta_zz(t+dt)
c
...
c
      do i=1,nsite
         sx(i,ia)=dcos(thez(i))*sx2(i)-dsin(thez(i))*sy1(i)
         sy(i,ia)=dsin(thez(i))*sx2(i)+dcos(thez(i))*sy1(i)
      end do
c      
c     theta_x(t+dt)
c
c      do i=1,nsite
c         sy(i,ia)=dcos(thex)*sy3(i)-dsin(thex)*sz2(i)
c         sz(i,ia)=dsin(thex)*sy3(i)+dcos(thex)*sz2(i)
c      end do
c
      return
      end
